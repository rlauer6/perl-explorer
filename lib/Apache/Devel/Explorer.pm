...
########################################################################
    ...
    if ( !$explorer || $EVAL_ERROR ) {
        my $config_path = $ENV{CONFIG} // q{};
        return html_error(
            $r,
            msg      => "$err<br>could not initialize E2E::Explorer...check your CONFIG path ($config_path)",
            explorer => $explorer
        );
    }
...
########################################################################
    ...
    my ( $msg, $status, $explorer ) = @{$options}{qw(msg status explorer)};

    my $params = {
        logo  => $explorer->get_config->{site}->{logo},
        error => { msg => $msg }
    };
...
END_OF_HTML
    ...
    my $output = tt_process( $html, $params );
...
########################################################################
    ...
    my $violations = eval { return $critic->critique(); };

    return
      if !$violations || $EVAL_ERROR;
...
########################################################################
    ...
    my ( $status, $content_type, $content, $pretty ) = @{$options}{qw(status content_type content pretty)};
